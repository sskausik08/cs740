...
class Topology(object):
	...
	def addLink(self, sw1, sw2) :
		if sw2 not in self.neighbours[sw1] : 
			self.neighbours[sw1].append(sw2)
		if sw1 not in self.neighbours[sw2] : 
	...
	def getPath(self, sw1, sw2) :
		...
		dstvisited = False
		while len(swQueue1) > 0 and not dstvisited  :
			for sw in swQueue1 : 
				neighbours = self.getSwitchNeighbours(sw)
				for n in neighbours : 
					if n == sw2 : 
						...
						dstvisited = True 
						break
					elif n not in swQueue2 and parent[n] == None : 
						parent[n] = sw
